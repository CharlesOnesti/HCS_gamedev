...
function love.load()
    ...
    function reset()
        ...
        gui.timer = 0;
        gameTimer = 0;
        if (gui.score > gui.high_score) then
...
function love.update(dt)
    ...
    if snakeAlive then
        ...
        if (gui.speed == true) then
            timerLimit = 0.13; -- snake speed
        else
            timerLimit = 0.06; -- snake speed
        end
        ...
        elseif directionQueue[1] == 'up' then
            ...
            if canMove then
                ...
                and snakeSegments[1].y == foodPosition.y then
                    if (gui.bonus == true) then
                        table.insert(snakeSegments, 1, {x = nextXPosition, y = nextYPosition})
                        table.insert(snakeSegments, 1, {x = nextXPosition, y = nextYPosition})
                        gui.score = gui.score + 2
                    end
                    if (gui.power == true) then
                        gui.speed = true;
                    else
                        gui.speed = false;
                    end
                    moveFood();
                    gui.score = gui.score + 1;
                    gui.rand = math.random(1, 10);
...
function love.draw()
    ...
    if (gui.rand > 5 and gui.rand < 8) then
        love.graphics.setColor(255, 215, 0); -- food colour
        gui.bonus = true;
        gui.power = false;
    elseif (gui.rand > 8) then
        love.graphics.setColor(0, 1, 1); -- food colour
        gui.power = true;
    else
...
function drawGUI()
  ...
  love.graphics.print("Best Score: " .. tostring(gui.high_score), 450, 425);
